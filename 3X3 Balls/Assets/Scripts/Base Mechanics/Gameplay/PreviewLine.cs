...
public class PreviewLine : MonoBehaviour
{
    ...
    private void OnRenderObject()
    {
        if (SceneInfo.instance.GameStart && SceneInfo.instance.IsAiming && gameObject == SceneInfo.instance.ActiveBall)
        {
            ...
            RaycastHit hitInfo;
            if (Physics.SphereCast(transform.position, GetComponent<SphereCollider>().radius * 0.85f, toCueBall, out hitInfo, firstDistance, mask))
            {
                //Limit the length
                ...
                if (hitInfo.transform.CompareTag("Ball"))
                {
                    ...
                    RaycastHit hitInfo2;
                    if (Physics.SphereCast(hitInfo.point, hitInfo.transform.GetComponent<SphereCollider>().radius * 0.85f, toOtherBall, out hitInfo2, secondDistance, mask))
                    {
                        //Limit the length
